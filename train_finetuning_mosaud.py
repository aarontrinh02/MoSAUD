...
from supe.models.dynamics import DynamicsModel
...
config_flags.DEFINE_config_file(
    "dynamics_config",
    "configs/dynamics_config.py",
    "File path to the dynamics model hyperparameter configuration.",
    lock_config=False,
)
...
def main(_):
    ...
    if hasattr(FLAGS, 'dynamics_config'):
        kwargs = dict(FLAGS.dynamics_config)
        model_cls = kwargs.pop("model_cls")
        dynamics_model = globals()[model_cls].create(
            FLAGS.seed + 456,
            meta_env.observation_space,
            meta_env.action_space,
            **kwargs,
    ...
    for i in tqdm.tqdm(
            ...
            if i < 20000:
                ...
                def estimate_value(state, actions, horizon=5):
                    ...
                    for t in range(horizon):
                        reward = rm.get_reward(state, actions[t])
                        state = dynamics_model.get_next_observation(state, actions[t])
                ...
                for t in range(5):
                    ...
                    z = dynamics_model.get_next_observation(z, policy_ac[t])
        ...
        if i >= FLAGS.start_training and dynamics_model is not None:
            dynamics_model, dynamics_info = dynamics_model.update(batch, FLAGS.utd_ratio)
            
            if i % FLAGS.log_interval == 0:
                for k, v in dynamics_info.items():
                    wandb.log(add_prefix("dynamics/", {k: v}), step=record_step)
