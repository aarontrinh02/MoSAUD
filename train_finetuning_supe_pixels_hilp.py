...
from supe.utils import (add_prefix, check_overlap,
                        ...
                        get_prior_statistics, view_data_distribution)
...
flags.DEFINE_string("project_name", "hilp-pixels", "wandb project name.")
...
def main(_):
    ...
    skill_agent_config = get_default_config(updates={"encoder": "impala_small_vam"})
    skill_agent_config.env_name = FLAGS.env_name
    hilp_agent_path = get_restore_path(FLAGS.env_name, seed=FLAGS.seed, visual=True)
    ...
    if FLAGS.use_rnd_offline or FLAGS.use_rnd_online:
        ...
        rnd = globals()[model_cls].create(
            FLAGS.seed + 123, observation_space, meta_env.action_space, **kwargs
        )
    else:
        rnd = None
